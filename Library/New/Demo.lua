...
-- srry :(
local Flow = loadstring(game:HttpGet("https://raw.githubusercontent.com/sscabtsv/Flow-Remastered/refs/heads/main/Library/New/Source.lua"))()

-- functions to simulate loading stuff up for tasks process in loading.
local function preload_assets()
  local waitTime = math.random(1, 3)
	task.wait(waitTime) -- pretend to preload images/sounds
end

local function fetch_player_data()
  local waitTime = math.random(0.3, 2.1)
	task.wait(waitTime) -- pretend to fetch saved data from a datastore/API
end

local function warm_up_modules()
  local waitTime = math.random(0.2, 1.1)
	task.wait(waitTime) -- pretend to initialize other modules/dependencies
end

local Window = Flow:CreateWindow({
	Title = "Flow | Reimagined", -- string
	Theme = "Aurora", -- string, choose from Markdowns/THEME_SYSTEM.md
	LoadingTitle = "Flow | Reimagined", -- string, shown on the themed loading screen
	...
	LoadingTasks = {
      ...
      fetch_player_data,
      warm_up_modules
  }, -- functions the loading screen waits on when LoadingMode is "tasks"
	NotificationPosition = "Down-Right", -- one of the 8 positions in the Notifications settings module
	NotificationSound = "Soft Ping", -- "None" or one of the built-in notification sounds
	Language = "English", -- any language in the Language settings module; the whole UI gets translated live
})
...
ColorMod:create_colorpicker({
	...
	flag = "HighlightColor",
	default = Color3.fromRGB(45, 220, 180),
...
UIMod:create_keybinddisplay({
	...
	binds = {
      "PlayerModule",
      "SprintBind"
  }, -- flags of any elements you want the keybind to control
})
...
local WatermarkMod = MiscTab:create_module({
	title = "Watermark",
	flag = "WatermarkMod",
	section = "left",
	callback = function() end,
...
WatermarkMod:create_checkbox({
	title = "Show Watermark",
	flag = "ShowWatermark",
	callback = function(state)
		if state and not activeWatermark then
			activeWatermark = Flow:CreateWatermark({
				title = "Flow | Reimagined", -- string, shown in the watermark pill
				fps = true, -- boolean, show live fps
				ping = true, -- boolean, show live ping
			})
...
for _, name in ipairs({"Aurora","Velvet","Solstice","Midnight","Cyber","Ocean","Sakura","Crimson","Glass"}) do
