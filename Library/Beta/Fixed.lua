...
local UIFontScale = 1;
local function measureFontHeight(family)
...
Library._apply_font_style = function(root)
	...
	local function styleOne(obj)
		...
		obj.TextSize = math.max(7, math.floor((base * UIFontScale) + 0.5));
	end
	styleOne(root);
...
Library.set_ui_font = function(self, fontEnum)
	...
	local baseHeight = measureFontHeight(GOTHAM_FAMILY);
	local newHeight = measureFontHeight(family);
	if (baseHeight and newHeight and (newHeight > 0)) then
		UIFontScale = math.clamp(baseHeight / newHeight, 0.7, 1.4);
	else
		UIFontScale = 1;
	end
	UIFontFamily = family;
