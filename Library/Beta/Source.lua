...
Library.create_ui = function(self)
	...
	ContainerGlow.Position = UDim2.new(0.5, 0, 0.5, 0);
	ContainerGlow.Size = UDim2.new(0, 0, 0, 0);
	...
	Connections['container_glow_pulse'] = RunService.Heartbeat:Connect(function(dt)
		...
		GlowStroke.Transparency = 0.5 + (wave * 0.25);
		GlowStroke.Thickness = 2 + (wave * 0.6);
	end);
	...
	local function update_drag(input)
		...
		TweenService:Create(ContainerGlow, TweenInfo.new(0.15, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Position=position}):Play();
	end
	...
	self.change_visiblity = function(self, state)
		...
		if state then
			...
			TweenService:Create(ContainerGlow, Tween.slow, {Size=UDim2.fromOffset(704, 485)}):Play();
		else
			if _divider then
			...
			TweenService:Create(ContainerGlow, Tween.slow, {Size=UDim2.fromOffset(minWidth + 6, 58)}):Play();
		end
	end;
