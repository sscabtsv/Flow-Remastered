...
- Added:
  + Loading Screen (testing)
    - Customizable Duration (fixed, tasks, modules)
    - fixed just uses a set duration (e.g 3)
    - tasks will use functions that needs waiting before initializing (e.g functions that uses task.wait()), it will load each function seperately based on duration (testing)
    - modules uses the arrangement of modules, it will load all modules, until it hits the line Window:load() (testing)
  + Watermark
    - Shows script name, FPS, and Ping
  + Font Changing
    - Added font management and changing, allowing for seamless font changing and management
    - Text auto sizes according to chosen font (testing)
  + Visual stuff
    - Added a glow border on the main window, for aesthetic purposes (testing)
  + Notification Sound
    - Added a fun feature that just plays a sound when notifications pop up.
  + Translation
    - Translates every text into your chosen language

- Changed/Fixed (Most of these are for optimization and notification issues)
  / Notification color theme
    - Old notifs did not match the theme when popped up, it only did when a new notif appeared
    - We fixed it, not if matches the theme on first pop up
  / Notification bar not rounding perfectly to the main notif frame
    - It used to spike out awkwardly
    - We fixed it by adjusting the size and corners of the bars
  / Normal Notifications (Different from error, warning, success) not matching the theme
    - Old unstable builds notification's bars didn't match the theme (issue only with info or loading), it was hardcoded only to default blue
    - Fixed it by running it through theme.Accent so the bars matched the theme.
  / Text being too small on some fonts
    - Old unstable builds had text that was way too small on some fonts
    - Fixed it by creating a method to simply resize the text if the pixels on the font are too small, and when changed to a big font, it'll switch to a smaller size.
  / Theme changing being laggy
    - On some occasions, theme changing will lag, especially when testing (environment was recording, max graphics, rapidly switching between themes)
    - Fixed it by optimizing everything about theme changing handler and some other optimizations 

- Removed
  - Herobrine
]]
-- Considered unstable since loading screen is still under testing
...
Library.create_settings_tab = function(self)
	...
	LanguageMod:create_label({title="Note",text="Translations are fetched live and cached per phrase, so the first pass after switching is the slowest."});
