...
Library.apply_theme = function(self, theme_name)
	...
	if elements.ContainerGlow then
		TweenService:Create(elements.ContainerGlow, ti, {BackgroundColor3=theme.Accent}):Play();
	end
...
Library.create_ui = function(self)
	...
	ContainerGlow.BackgroundColor3 = Themes[Library._current_theme].Accent;
	ContainerGlow.BackgroundTransparency = 0.88;
	...
	self.change_visiblity = function(self, state)
		...
		if state then
			...
			TweenService:Create(ContainerGlow, Tween.slow, {Size=UDim2.fromOffset(730, 511)}):Play();
		else
			if _divider then
			...
			TweenService:Create(ContainerGlow, Tween.slow, {Size=UDim2.fromOffset(minWidth + 32, 84)}):Play();
		end
	end;
